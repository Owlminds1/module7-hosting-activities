...
import data from "@/src/layout-C50-L2-warmup/data.json";
import { useEffect, useState } from "react";

type ItemType = {
  id: string;   // fixed type
  Name: string;
};

export default function LayoutC50L2Warmup() {
  ...
  return (
    <div className="py-8 min-h-screen bg-[#F8FCFA] flex items-center justify-center gap-3 pt-5 flex-col">
      ...
      <div className="flex justify-center item-center flex-wrap px-8 gap-4">
        <button className="bg-yellow-600 text-lg rounded py-2 px-8 active:scale-95"
          onClick={moveUp}>
          Move up
        </button>
        ...
        <button className="bg-green-600 text-lg rounded py-2 px-5 active:scale-95"
          onClick={checkResult}>
          Check result
        </button>
      </div>
      ...
      {items.map((item, index) => (
        <h1
          ...
          className={`${indexItem === index ? "border-3 border-black" : ""} 
